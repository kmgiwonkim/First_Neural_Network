...
# images.shape(240000, 28, 28) labels.shape (240000,)
from emnist import extract_test_samples
images, labels = extract_test_samples('digits')
images.shape(40000, 28, 28)labels.shape(40000,)
import pickle
...
class Variables:
    ...
    def __init__(self, n, eta, batchSize, repetitions, Dataset):
    ...
    def imagesSetup(self, Dataset):
        ...
        if Dataset == 0:  # MNIST Setup
            ...
            print('.', end='')
            images = self.mndata.load_training()
            self.images = [np.array([np.ravel(np.transpose([np.reshape(images[0][x], (28, 28))])), images[1][x]]) for x
                           in range(len(images[0]))]
            print('.', end='')
            self.mndata = MNIST(self.dSetIndex[Dataset])
            imagesTe = self.mndata.load_testing()
            self.imagesTe = [np.array([np.ravel(np.transpose([np.reshape(imagesTe[0][x], (28, 28))])), imagesTe[1][x]])
                             for x in range(len(imagesTe[0]))]
